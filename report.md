> **文件用途：** 本文件為 Computer Architecture Final Project 報告／簡報前六個核心內容的 Notion 草稿，涵蓋研究動機、OFDM workload、數學模型、實驗環境、驗證方法與 Part 1 scalar baseline。可直接貼到 Notion，再轉為投影片或正式 `Report.pdf`。
> 
> 
> **目前完成範圍：** GitHub repository 目前已完成 Part 1～Part 3。本份草稿先建立共同 workload 與 Scalar Baseline；Part 2 RVV Reduction 與 Part 3 SIMD-like RVV 的細節、比較與討論可在此基礎上接續補入。
> 
...
另外，Part 4 / Part 5 所需的 CUDA 開發環境也已完成驗證。主機端可使用 `NVIDIA GeForce RTX 3060`，Compute Capability 為 `8.6`；Docker GPU runtime 可正常使用，且 CUDA container 內已確認 `nvcc 12.4`、`-arch=sm_86` 編譯、CUDA kernel launch、CUDA event timing、PTX generation、PTXAS resource report、`Nsight Compute (ncu)` 與 GPU performance counters 皆可正常運作。需特別說明的是：主機驅動顯示的 `CUDA Version` 為 `13.0`，但本專題實際使用的作業 container 內工具鏈為 `CUDA Toolkit 12.4.1`；兩者在目前系統上相容，且已透過 `cuda_smoke` 與 `ncu` 實測驗證可用。因此目前不需要再額外建置 CUDA 環境，可以直接開始 Part 4 與 Part 5 的實作。對應的 `part4/` 與 `part5/` 目錄也已建立完成。
