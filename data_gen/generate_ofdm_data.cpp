...
static std::normal_distribution<float> awgn_dist(0.0f, NOISE_STD);
...
int main(int argc, char **argv)
{
    ...
    std::fwrite(htrue_r, sizeof(float), NUM_SUBCARRIERS, fp);
    std::fwrite(htrue_i, sizeof(float), NUM_SUBCARRIERS, fp);
    std::fwrite(pilot_w_local, sizeof(float), NUM_PILOTS, fp);
    std::fwrite(ypilot_r, sizeof(float), TOTAL_PILOT, fp);
    std::fwrite(ypilot_i, sizeof(float), TOTAL_PILOT, fp);
    std::fwrite(xdata_r, sizeof(float), TOTAL_DATA, fp);
    std::fwrite(xdata_i, sizeof(float), TOTAL_DATA, fp);
    std::fwrite(ydata_r, sizeof(float), TOTAL_DATA, fp);
    std::fwrite(ydata_i, sizeof(float), TOTAL_DATA, fp);
    std::fclose(fp);

    std::printf("Generated OFDM input: %s\n", filename);
